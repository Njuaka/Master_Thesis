...
class BalancedAccuracy(tf.keras.metrics.Metric):
    ...
    #def update_state(self, y_true, y_pred, sample_weight=None):
        # Round each element of y_pred to the nearest integer
        #y_pred_rounded = tf.round(y_pred)

        # Compute the confusion matrix for each label
        #confusion_matrices = [tf.math.confusion_matrix(y_true[:, i], y_pred_rounded[:, i], num_classes=self.noc)
                          for i in range(self.noc)]

        # Compute the sum of the confusion matrices
        #confusion_matrix_sum = tf.add_n(confusion_matrices)

        return self.confusion_matrix.assign_add(confusion_matrix_sum)

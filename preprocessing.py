...
class dataset_wrapper:
    def __init__(self, h5fp, labels, channels):
        ...
        for i, chan in enumerate(channels):
            ...
            np.subtract(self.images[i].T, min_, out=self.images[i].T)
            np.divide(self.images[i].T, min_ + max_, out=self.images[i].T)
...
def load_dataset(data, indices, labels, args, type="train", augment_func=None):
    ...
    if type=="train":
        X = tf.data.Dataset.from_generator(
            generator(data, indices, shuffle=True), 
            output_types=(tf.float32, tf.float32), 
            output_shapes=(tf.TensorShape([3,90,90]), tf.TensorShape([labels.shape[1]]))
        ).repeat()
            
        ds = X.batch(batch_size=args["batch_size"])
        ...
        if type =="val":
            ...
            ds = tf.data.Dataset.from_generator(
                X, output_types=(tf.float32, tf.float32),output_shapes=(tf.TensorShape([3,90,90]), tf.TensorShape([labels.shape[1]]))
            )
            ds_length = len(indices)
        else:

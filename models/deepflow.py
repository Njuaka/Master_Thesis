...
def deepflow(args):
    ...
    #leaky_relu_alpha = 0.01   # usually a small, positive number
    ...
    def _dual_factory(inp, f_out_1, f_out_2):
        with tf.name_scope("Dual") as scope:
            ...
            with tf.name_scope("Left") as scope:
                ...
                #conv1 = LeakyReLU(alpha=leaky_relu_alpha)(conv1)
            ...
            with tf.name_scope("Right") as scope:
                ...
                #conv2 = LeakyReLU(alpha=leaky_relu_alpha)(conv2)
    ...
    with tf.name_scope("Classifier") as scope:
        ...
        #fc1=LeakyReLU(alpha=leaky_relu_alpha)(fc1)
        layer_name = "dense_"+str(args["noc"])
        last = keras.layers.Dense(
        args["noc"], activation="sigmoid", kernel_regularizer=keras.regularizers.l2(l=args["l2"]),
        kernel_initializer=initializer, bias_initializer=initializer, name=layer_name)(fc1)

        #if args["noc"] == 208:
            # Use sigmoid instead of softmax
            #last = keras.layers.Dense(
                #208, activation="sigmoid", kernel_regularizer=keras.regularizers.l2(l=args["l2"]),
                #kernel_initializer=initializer, bias_initializer=initializer, name=layer_name)(fc1)
        #else:
            #last = keras.layers.Dense(
               # args["noc"], activation="sigmoid", kernel_regularizer=keras.regularizers.l2(l=args["l2"]),
...
def deepflow_narrow(args):
    ...
    #fc1=LeakyReLU(alpha=leaky_relu_alpha)(fc1)
    soft = keras.layers.Dense(args["noc"], activation="sigmoid", kernel_regularizer=keras.regularizers.l2(l=args["l2"]))(fc1)
